...
def eval_val(
    ...
    with torch.no_grad():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            prev_ids = local[:-1].reshape(-1)
            tgt_ids = local[1:].reshape(-1)
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda",
    ).split(",")
    if pattern
)
...
class DistributedTokenLoader:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor | None]:
        local_tokens = global_tokens // (self.world_size * grad_accum_steps)
        per_rank_span = local_tokens
        chunk = self.stream.take(per_rank_span * self.world_size)
        start = self.rank * per_rank_span
        local = chunk[start : start + per_rank_span].to(dtype=torch.int64)
        tokens = local.reshape(-1, seq_len)
        return tokens.to(self.device, non_blocking=True), None
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        y = flash_attn_3_func(q, k, v, causal=False)
        if self.use_xsa:
...
def generate_diffusion_calib(model, device, num_seqs=64, seq_len=2048, vocab_size=1024, seed=42):
    ...
    with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        for _ in range(0, num_seqs, 8):
            bs = min(8, num_seqs - len(all_tokens))
            tokens = torch.randint(0, vocab_size, (bs, seq_len), device=device, generator=rng)
            t = 0.5
            mask = torch.rand(tokens.shape, device=device, generator=rng) < t
            tokens[mask] = vocab_size
            for i in range(bs):
...
def main() -> None:
    ...
    log0("gptq:generating diffusion calibration data (64 seqs x 2048 tokens)...")
    base_model.load_state_dict(export_sd, strict=False)
    t_gen = time.perf_counter()
    calib_tokens = generate_diffusion_calib(
    ...
    log0("gptq:collecting hessians from diffusion data...")
    hessians = collect_hessians_from_tokens(hessian_model, calib_tokens, device)
    log0(f"gptq:collected hessians for {len(hessians)} layers (Diffusion self-gen)")
    del calib_tokens
    del hessian_model
